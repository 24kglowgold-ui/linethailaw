...
async function initDB() {
    try {
        ...
        // Initialize TTL Index: Automatically delete messages after 1 hour (3600 seconds)
        await db.collection("conversations").createIndex({ "timestamp": 1 }, { expireAfterSeconds: 3600 });
    } catch (err) {
...
function createGreetingFlex(lang) {
    ...
    const translations = {
        'thai': {
            body: "ยินดีต้อนรับ! ผมพร้อมช่วยตอบคำถามกฎหมายไทยของคุณ คำตอบทั้งหมดสร้างโดย AI และไม่ใช่คำแนะนำทางกฎหมาย โปรดทราบว่าข้อมูลการสนทนาจะถูกเก็บรวบรวมและถูกลบภายใน 1 ชั่วโมง",
            policyLabel: "อ่านนโยบาย",
            acceptLabel: "ยอมรับ",
            acceptText: "เริ่มแชท"
        },
        'chinese': {
            body: "欢迎！我在这里为您解答有关泰国法律的问题。所有回答均由 AI 生成，不构成法律建议。请注意，对话数据将被收集并于 1 小时内删除。",
            policyLabel: "阅读政策",
            acceptLabel: "我接受",
            acceptText: "开始聊天"
        },
        'english': {
            body: "Welcome! I'm here here to help with your Thai legal queries. All answers are generated by Ai and they are not legal advice. Please note the conversational data will be collected and deleted in 1 hour.",
...
async function handleEvent(event) {
    ...
    const historyDocs = await db.collection("conversations")
        .find({ userId: userId })
        .sort({ timestamp: -1 })
        .limit(20)
        .toArray();

    // 2. Format history for Gemini (mapping "assistant" role to "model")
    ...
    const callGemini = async (retries = 3) => {
        try {
            // Note: Updated model name to gemini-1.5-flash for stability
