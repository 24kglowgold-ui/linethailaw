...
const LANGUAGES = {
    ...
    'english': {
        ...
        limitText: "10 free questions used. Subscribe for unlimited access.", // Shortened to fit 60-char limit
    ...
    'chinese': {
        ...
        limitTitle: "已达到每日上限",
        limitText: "您已达到10个问题的免费额度。请订阅。", // Shortened
        payButton: "支付 20 泰铢",
        disclaimer: "\n\n**免责声明：** 本信息不构成正式法律建议。",
        ctaLabel: "咨询律师"
    },
    'traditional_chinese': {
        ...
        limitTitle: "已達到每日上限",
        limitText: "您已達到10個問題的免費額度。請訂閱。", // Shortened
        payButton: "支付 20 泰銖",
        disclaimer: "\n\n**免責聲明：** 本信息不構成正式法律建議。",
        ctaLabel: "諮詢律師"
    }
...
async function saveChatTurn(userId, userMessage, aiResponse) {
    ...
    try {
        await db.collection("history").insertOne({
            userId, userMessage, aiResponse, timestamp: new Date()
        });
    } catch (err) {}
}
...
async function handleEvent(event) {
    ...
    if (userMessageCounts[userId] >= 10) {
        const limitMsg = {
            ...
            template: {
                type: 'buttons',
                title: lang.limitTitle, // Max 40 chars
                text: lang.limitText,   // Max 60 chars
                actions: [{ 
                    type: 'uri', 
        ...
        return await client.replyMessage({ replyToken, messages: [limitMsg] })
            .catch(async () => {
                await client.pushMessage({ to: userId, messages: [limitMsg] });
            });
    }
