...
const cron = require('node-cron'); // Required for the daily reset schedule
...
// --- DAILY RESET LOGIC ---
// This runs every day at 00:00 (Midnight) Bangkok time
cron.schedule('0 0 * * *', () => {
    console.log('--- Triggering Midnight Reset (Bangkok Time) ---');
    // Wipes the message counts for every user in memory
...
const LANGUAGES = {
    ...
    'english': {
        ...
        // FIX 1: Ensure text is within character limits
        limitText: "Free limit reached. Subscribe for unlimited daily access.",
        // FIX 2: Shortened label (Max 20 characters) to avoid 400 Bad Request
...
// FIXED WEBHOOK: Returns OK immediately to stop LINE from retrying/looping
...
async function handleEvent(event) {
            ...
            messages: [{
                ...
                template: {
                    ...
                    actions: [{ 
                        ...
                        // FIX 3: Corrected URL syntax with the variable
                        uri: 'https://CNvTn8.short.gy/Z3S9Vo?userId=' + userId 
                    }]
                }
            }]
        }).catch(err => {
            // Log full error details to help debug if another 400 occurs
            console.error("Limit Reply Error:", err.response?.data || err.message);
        });
    }
    ...
    const callGemini = async (retries = 3) => {
        try {
            return await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key=${process.env.GEMINI_API_KEY}`, {
                ...
                generationConfig: { thinkingConfig: { thinkingLevel: "low" }, temperature: 1.0 },
                tools: [{ "google_search": {} }],
    ...
    try {
        ...
        // Solve reply token expiration by using Push Message as a fallback
