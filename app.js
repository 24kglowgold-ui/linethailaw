...
// --- TRACKING AND RESET LOGIC ---
const userMessageCounts = {};
let lastResetDate = new Date().getUTCDate(); // Stores the current day of the month

function checkAndResetLimits() {
    const today = new Date().getUTCDate();
    if (today !== lastResetDate) {
        console.log("New day detected. Resetting all user message counts.");
        for (let userId in userMessageCounts) {
...
async function handleEvent(event) {
    ...
    // LIMIT CHECK WITH PUSH FALLBACK TO AVOID 400 ERROR
    if (userMessageCounts[userId] >= 5) {
        ...
        return await client.replyMessage({ replyToken: replyToken, messages: [limitMessage] })
            .catch(async (err) => {
                // If token expired (400 error), send via Push instead
                await client.pushMessage({ to: userId, messages: [limitMessage] });
            });
    }
