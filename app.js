...
async function handleEvent(event) {
    ...
    // --- MEMORY FEATURE: FETCH LAST 20 MESSAGES ---
    const pastMessages = await db.collection("conversations")
        .find({ userId: userId })
        .sort({ timestamp: -1 })
        .limit(20)
        .toArray();

    // Format for Gemini API (Reverse to chronological order and map roles correctly)
    ...
    const callGemini = async (retries = 3) => {
        try {
            return await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key=${process.env.GEMINI_API_KEY}`, {
                contents: history, // Memory context is passed here instead of just the single message
