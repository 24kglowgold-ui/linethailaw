...
// Memory storage for counts and the current tracked date
let userMessageCounts = {};
let lastResetDate = new Date().toLocaleDateString("en-US", { timeZone: "Asia/Bangkok" });

// SMART RESET FUNCTION: Checks if the date has changed
function checkAndResetDailyCounts() {
    const today = new Date().toLocaleDateString("en-US", { timeZone: "Asia/Bangkok" });
    
    if (today !== lastResetDate) {
        console.log(`--- New Day Detected (${today}). Resetting all counts. ---`);
        userMessageCounts = {}; // Wipe all counts
        lastResetDate = today;   // Update the tracked date
    }
}
...
// --- NEW REVISION: HEALTH CHECK ROUTE ---
// Render needs this GET route to verify the service is running
app.get('/', (req, res) => {
...
app.post('/webhook', line.middleware(config), (req, res) => {
    ...
    // Perform the smart reset check before processing any events
...
// --- NEW REVISION: PORT BINDING ---
// Explicitly binding to '0.0.0.0' allows Render to detect the open port
